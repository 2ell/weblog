Type: File
Content-Type: text/css
Title: Stylesheet
Location: /style.css

:root {
    --v-faint: oklch(100% 0 0 / 4%);
    --faint: oklch(100% 0 0 / 30%);
	--foreground-faded: oklch(97.75% 0.04 106.55 / 20.69%);
	--background: oklch(28.82% 0.022 277.51);
    --main-color: oklch(40.28% 0.032 277.83 / 100%);
    --faint-text: oklch(52.79% 0.024 279.54);
    --foreground: oklch(97.75% 0.008 106.55);
    --comment: oklch(55.98% 0.08 270.09);
    --cyan: oklch(88.26% 0.093 212.85);
    --green: oklch(87.1% 0.22 148.02);
    --orange: oklch(83.39% 0.124 66.56);
    --pink: oklch(75.46% 0.183 346.81);
    --purple: oklch(74.2% 0.09 301.88);
    --red: oklch(68.22% 0.206 24.43);
    --yellow: oklch(95.53% 0.134 112.76);
	--gradient1: linear-gradient(145deg, var(--yellow), var(--purple), var(--cyan));
	--gradient2: linear-gradient(120deg, var(--lavendar), var(--teal), var(--yellow), var(--pink));
	--gradient3: linear-gradient(120deg, var(--blue), var(--sky), var(--teal));
	--default-font: 'Lato', sans-serif;
	--header-font: 'VC Honey Deck', serif;

	--default-size: 1.2em;
	--border-radius: 0.7em;
}

* {
    margin: 0 auto;
    overflow: ;
}

.gradient-frame {
        position: sticky;
        position: -webkit-sticky;
	    top: 0 !important;
	    left: 0;
        background-image: var(--gradient1);
        width: 100%;
        height: fit-content;
        padding-bottom: 0.2rem;
        color: var(--foreground);
        box-shadow: 
		rgba(0, 0, 0, 0.1) 0px 1px 1px, 
		rgba(0, 0, 0, 0.01) 0px -12px 30px, 
		rgba(0, 0, 0, 0.1) 0px 4px 6px, 
		rgba(0, 0, 0, 0.1) 0px 12px 13px, 
		rgba(0, 0, 0, 0.03) 0px -3px 5px;
        z-index: 999;
    }

.top {
	min-height: 2.4em;
	background-color: var(--background);
	width: 100%;
}
    .top-container {
        display: flex;
        align-items: flex-end;
        max-width: 50em;
    }

	.top-left {
		flex: 1;
		flex-grow: 4;
        margin-left: 1rem;
        margin-top: 1rem;
	}

		.top-title a {
			color: var(--foreground);
			background-image: var(--gradient1);
			background-size: 0;
			-webkit-background-clip: text;
			background-clip: text;
			transition: all 0.6s ease-in-out;
			padding-bottom: 0;
			border-bottom: none;
		}

		.top-title a:hover {
			background-size: 100%;
			background-blend-mode: lighten;
			color: transparent;
			-webkit-background-clip: text;
    		background-clip: text;
			transition: all 0.6s ease;
			border-bottom: none;
		}

	.top-right {
		margin-top: 0.5rem;
		flex: 2;
        flex-grow: 6;
        align-self: flex-end;
        text-align: right;
	}

	.top-right ul {
	    list-style-type: none;
	    text-align: right;
	}

	.top-right li {
	    display: inline-flex;
    }

    .top-right a {
        padding-right: 1rem;
        border-bottom: 0 !important;
    }



body {
    font-family: 'Lato', sans-serif;
    font-size: var(--default-size);
    padding: 0;
    background-color: var(--background);
    color: var(--foreground);
    }

    h1, h2, h3, h4, h5, h6 {
        font-family: 'VC Honey Deck', serif;
    }

    h3 {
        font-size: 1.3em;
    }

    a {
        text-decoration: none !important;
        color: var(--foreground);
        background-image: var(--gradient1);
        background-size: 0;
        -webkit-background-clip: text;
        background-clip: text;
        border-bottom: 0.2rem solid var(--v-faint) !important;     
        transition: all 0.2s ease-in-out;
        padding-bottom: 0;
    }
        a:hover {
            background-size: 100%;
            color: transparent;
            -webkit-background-clip: text;
            background-clip: text;
            border-bottom: 0.2rem solid var(--faint);
            transition: all 0.2s ease;
        }

    hr {
        background-image: var(--gradient1);
        background-size: 100%;
        padding: 0.5px;
        border: 0;
    }

    /* special classes */

    .gradient-bg {
    max-width: fit-content;
    margin: auto;
    border-radius: var(--border-radius) var(--border-radius);
    transform: translate(0);
    background-color: var(--main-color);
    z-index: -999;
}
    .gradient-bg:before {
        content:"";
        position: absolute;
        inset: 30px;
        transform: translate(0.5em, 0.5em);
        background: var(--gradient1);
        opacity: 0.5;
        filter: blur(2rem);
        z-index: -999;
    }

    .center {
    text-align: center;
    }

    .no-btm {
        margin-bottom: 0;
        padding-bottom: 0;
    } 

    .h-dark {
        font-family: 'VC Honey Deck', serif;
        color: #282a36;
    }

    .h-purple {
    font-family: 'VC Honey Deck', serif;
    color: var(--purple);
    }

    .small {
    font-size: 0.8em;
    list-style-type: none;
    }

    /* sections: top ------- */

    main {
        max-width: 50em;
    }

    .main {
        background-color: var(--main-color);
        border-radius: var(--border-radius);
        padding-bottom: 1rem;
    }

    .wrapper {
        margin-top: 3rem;
        padding-top: 2rem;
        padding-bottom: 0;
        margin-bottom: -0.5rem;
        display: flex;
        gap: 0.2rem;
        height: fit-content;
        max-width: 90%;
    }

    .wrapper-left {
        flex: 1;
        flex-grow: 4;
        align-self: flex-end;
    }

    .wrapper-right {
        flex: 2;
        flex-grow: 6;
    }

.content {
		margin-top: 2rem;
		margin-bottom: 2rem;
	}

main p {
	margin: 1rem 0 1rem;
}

.article-container {
	margin: .5rem .5rem 3rem;
}

article {
	padding-top: 2rem;
	padding-bottom: 1rem;
}

.post-info-container {
	padding-bottom: 1rem;
	margin-bottom: 1rem;
	width: 100%;
	height: fit-content;
	border-bottom: 1px solid var(--purple);
	float: right;
}

	.post-info {
		text-decoration: none !important;
		font-size: 0.9rem;
		padding: 0;
		margin-bottom: 0;
		float: right;
		text-align: right;
	}

.post-tags-container {
	margin-top: 2rem;
	padding-top: 1rem;
	width: 100%;
	border-top: 1px solid var(--crust);
}

	.recent {
	color: var(--background);
	max-width: 30em;
  	padding: 2rem 2rem 1rem;
	border-bottom: 0;
	background-color: var(--purple);
	border-radius: 0.7em;
	margin-bottom: -1rem;
}

.recent-posts {
	margin: 1rem 1rem 3rem;
}

    footer {
        color: var(--foreground);
        background-color: var(--main-color);
        max-width: 100%;
        padding: 2em;
        text-align: center;
    }

    footer a {
        text-transform: lowercase;
        color: var(--foreground);
        font-family: 'VC Honey Deck', serif;
        border-bottom: 0px !important;
        padding-top: 0.5rem;
    }

    .credits {
        font-size: 1em;
    }

    @media (max-width: 500px) {
	body {
		font-size: 1em;
	}
	main {
		padding: 0.3rem;
	}
    .section {
        gap: 0;
        margin: 0;
        padding-left: 0.2rem;
    }
    .box-connect {
        flex-grow: 1;
        border-right: 2px solid var(--main-color);
        line-height: 1.6rem;
        padding-left: 1rem;
    }
    .box-connect h3 {
      font-size: 2em;
    }
    .box-status {
        padding-right: 1rem;
        font-size: 1em;
        border-left: 2px;
    }
    .box-status h3 {
        display: none;
    }
    #laurel {
        padding-top: 0rem;
    }
}